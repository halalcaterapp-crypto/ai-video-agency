...
def _hf_submit_and_poll(endpoint: str, payload: dict) -> dict:
    """Submit a job to Higgsfield and poll until complete. Returns final response dict."""
    url = f"{HF_BASE}/{endpoint}"
    headers = _hf_headers()

    logger.info("Submitting to Higgsfield: %s", url)
    resp = requests.post(url, json=payload, headers=headers, timeout=60)
    logger.info("Higgsfield response %d: %s", resp.status_code, resp.text[:300])
    resp.raise_for_status()
    data = resp.json()
    ...
    request_id = (data.get("id") or data.get("request_id") or
                  data.get("requestId") or data.get("data", {}).get("id"))
    if not request_id:
        raise ValueError(f"No request_id in response and no immediate result: {data}")

    logger.info("Job submitted, request_id=%s — polling...", request_id)
...
def _extract_url(data: dict) -> str:
    """Extract any URL from a Higgsfield response dict."""
...
def generate_shot_clip(scene_number: int, prompt: str, duration_seconds: int, job_dir: str) -> str:
    ...
    _download(image_url, keyframe_path)
